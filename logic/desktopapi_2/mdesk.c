...
void MDesk_SwitchDesktopsWithSafeMode(PMDESK_INTERNAL pMdesk, DWORD dwDesktop)
{
  ...
  pMdesk->hWinPosInfo = BeginDeferWindowPos(0x10);
  
  EnumWindows(MDesk_SaveDesktopSafeMode, (LPARAM)pMdesk);     
  
  EndDeferWindowPos(pMdesk->hWinPosInfo);
...
BOOL CALLBACK MDesk_SaveDesktopSafeMode(HWND hWnd, LPARAM lParam)
{
    ...
    if(IsWindowVisible(hWnd) && pMdesk->hProgman != hWnd && pMdesk->hTaskbar != hWnd)
    {
       if(!MDesk_IsUniversalWindow(pMdesk, hWnd))
       {
         if(MDesk_SaveWindow(pMdesk, hWnd))
             pMdesk->hWinPosInfo = DeferWindowPos(pMdesk->hWinPosInfo, hWnd, 0, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOZORDER | SWP_HIDEWINDOW);
       }
    }
  ...
  while(pWindowWalker)
  {
     if(pWindowWalker->dwDesktop == pMdesk->dwCurrentDesktop)
     {
        if(IsWindow(pWindowWalker->hWnd))
           pMdesk->hWinPosInfo = DeferWindowPos(pMdesk->hWinPosInfo, pWindowWalker->hWnd, 0, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOZORDER | SWP_SHOWWINDOW | SWP_NOACTIVATE);
